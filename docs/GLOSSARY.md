...
| **bridge** | `firmware/espnow_bridge/` -- a reference sketch, marked NOT TESTED ON HARDWARE | M5Stack CoreS3 in Cambium binary-modem mode; PowerFeather `F2BED4` is the fallback | `bridge.ts` -- the browser-side client for JSON lines over serial | The physical USB/ESP-NOW modem whose serial port the daemon owns; CoreS3 is primary |
